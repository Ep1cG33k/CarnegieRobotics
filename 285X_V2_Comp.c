#pragma config(Sensor, in1,    lineFollowRight, sensorLineFollower)
...
#pragma config(Motor,  port5,           mBackLift,     tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port6,           mFrontLift,    tmotorVex393_MC29, openLoop)
...
void leftMotor(int power) {
	motor[port1] = motor[port2] = motor[port3] = motor[port4] = power;
}
...
void liftF(int power, int time){
	motor[port6] = power;
	wait1Msec(time);
	motor[port6] = 0;
}
void liftB(int power, int time){
	motor[port5] = power;
	wait1Msec(time);
	motor[port5] = 0;
}
task usercontrol()
{
	...
	while (true)
	{
		...
		if (driveMode) {
			motor[port1] = motor[port2] = motor[port3] = motor[port4] = vexRT[Ch3];
			motor[port7] = motor[port8] = motor[port9] = motor[port10] = -vexRT[Ch2];
			} else {
			motor[port1] = motor[port2] = motor[port3] = motor[port4] = -vexRT[Ch2];
			motor[port7] = motor[port8] = motor[port9] = motor[port10] = vexRT[Ch3];
		}
		if (vexRT[Btn7L]) {
			while(vexRT[Btn7L]){}
			driveMode = !driveMode;
